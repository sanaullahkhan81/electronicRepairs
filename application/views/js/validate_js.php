...
function checkLength(o, n, min, max, errorElement) {
	if (o.val().length > max || o.val().length < min) {
		updateTips("<?= $this->lang->line('val_ilc');?> " + n + " <?= $this->lang->line('val_deveal');?>  " + min + " <?= $this->lang->line('val_char');?>", errorElement);
		return false;
	} else {
		return true;
	}
...
function validNumeric(alpha, errorElement, message) {
        return checkRegexp(alpha, /^(\d+(?:[\.\,]\d{2})?)$/i, message, errorElement);
}
